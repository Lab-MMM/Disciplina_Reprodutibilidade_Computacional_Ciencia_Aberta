...
```{r echo=FALSE,eval=TRUE,out.width="55%",fig.cap="Uma visão geral de um projeto versionado. Ilustração feita por [Allison Horst](https://allisonhorst.com/git-github)"}
...
```{r echo=FALSE,eval=TRUE,fig.cap="Referência de velho. Para quem quiser saber mais sobre a novela, [veja aqui](https://pt.wikipedia.org/wiki/O_Clone)",out.width="55%"}
...
```{r echo=FALSE,eval=TRUE,fig.cap="Copiando a chave https: um dos protocolos para conectar seu repositório local ao remoto"}
...
Ao clicar em `Create project` o repositório remoto será imediatamente transferido para o seu computador. Uma pasta (o diretório local) será criado no local que você escolheu e, assim, RStudio, Git e Github estarão plenamente conectados. Tudo certo até aqui? 
    Deixo um vídeo rápido que trata da importância da reprodutibilidade.

```{r echo=FALSE,eval=TRUE,fig.cap="Um vídeo breve a crise da reprodutibilidade"}
...
```{r echo=FALSE,eval=TRUE,fig.cap="Arquivo de sistema indicando que o git está funcionando no seu diretório"}
...
```{r echo=FALSE,eval=TRUE,out.width="55%",fig.cap="Git está te observando, mas pra te ajudar"}
...
```{r echo=FALSE, eval=FALSE,out.width="60%",fig.cap="Os commits são um componente chave no versionamento, como destacado nessa ilustração da Allison Horst, são fundamentais para manter a rota de desenvolvimento de um projeto"}
knitr::include_graphics("figuras/commit-only-concept.png")
```
...
```{r echo=FALSE, eval=TRUE,out.width="70%", fig.cap="Commits são essenciais não só para o proprietário do repositório, mas também para auxiliar outras pessoas a compreenderem os caminhos para obtenção dos resultados, como ilustrado nessa figura da Allison Horst"}
